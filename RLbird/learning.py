...
def main():
    global HITMASKS, ITERATIONS, ANALYSIS, bot

    parser = argparse.ArgumentParser("learn.py")
    parser.add_argument("--iter", type=int, default=1000, help="number of iterations to run")
    parser.add_argument("--analysis", action="store_true", help="show scatter of training result")
...
def mainGame(movementInfo):
    ...
    while True:
        ...
        if crashTest[0]:
            # Update the q scores when game's over
...
def showGameOverScreen(crashInfo):
    ...
    if ANALYSIS:
        csv.append([bot.gameCNT, score])
    if bot.gameCNT == (ITERATIONS):
        bot.dump_qvalues(force=True)
        columns = ["ITERATIONS", "SCORE"]
        if ANALYSIS:
            print("dump analysis into .csv")
            file = pd.DataFrame(csv, columns=columns)
            file.to_csv("analysis.csv", index = 0)
        sys.exit()
