...
class Bot(object):
    ...
    After every iteration updates Q values
            ...
            for x in chain(list(range(-40, 140, 10)), list(range(140, 421, 70))):
                for y in chain(list(range(-300, 180, 10)), list(range(180, 421, 60))):
                    for v in range(-10, 11):
                        qval[str(x) + "_" + str(y) + "_" + str(v)] = [0, 0] #act,value
    ...
    def act(self, xdif, ydif, vel):
        """
        Chooses the best action with respect to the current state - Chooses 0 (don't flap)
        """
        ...
        if self.qvalues[state][0] >= self.qvalues[state][1]: #  refer Q-table
            self.last_action = 0
            return 0
        else:
            self.last_action = 1
    ...
    def update_scores(self, dump_qvalues=True):
        ...
        self.gameCNT += 1
        if dump_qvalues:
            self.dump_qvalues()  # Dump q values
        self.moves = []  # clear history after updating

    def map_state(self, xdif, ydif, vel):
        """
	In order to lower the number of states:
	For state that the bird close to the pipe, divided into 10x10 grids
	For state that far away from pipe, divided into bigger grids
    ...
    def dump_qvalues(self, force=False):
        ...
        """
        if (self.gameCNT - 1) % self.DUMPING_N == 0 or force:
